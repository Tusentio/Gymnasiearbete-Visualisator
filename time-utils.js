const MILLIS_WEEK = 6048e+5;
const MILLIS_DAY = 864e+5;
const MILLIS_MINUTE = 6e+4;
...
class WeekDate {
  constructor(year, ...args) {
    ...
    if (args.length < 2) {
      ...
      let [week, day] = args;
      this._timeSinceWeekYearStart = week * MILLIS_WEEK + day * MILLIS_DAY;
    }
  }
  ...
  getDay() {
    return Math.floor(this._timeSinceWeekYearStart % MILLIS_WEEK / MILLIS_DAY);
  }
  ...
  static parse(string) {
    let groups = /^([0-9]{4})(-?)W(0[1-9]|[1-4][0-9]|5[0-3])\2([1-7])?$/.exec(string);
    if (!groups)
      throw new Error(`Ogiltigt veckodatum: ${string}. (ISO 8601 week date)`);
...
function parseTime(time) {
  let groups = /^([0-1]?[0-9]|2[0-9])(:?)([0-5][0-9])(?:\2([0-5][0-9])(?:\.([0-9]{3}))?)?$/.exec(time);
  if (!groups)
    throw new Error(`Ogiltigt tidsformat: ${time}.`);
